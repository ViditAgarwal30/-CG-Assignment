...
void Parent(TREENODE* tree, int myid, TREENODE** parent)
{
	if (tree->id == (myid / 2))
		*parent = tree;
	if (tree->left)
		Parent(tree->left, myid, parent);
	if (tree->right)
		Parent(tree->right, myid, parent);

}
void addNode(TREENODE** t_root, TREENODE* current) {
	TREENODE* parent;
	if (current->id == 1) {
		*t_root = current;
	}
	...
	{
		Parent(*t_root, current->id, &parent);
		if ((current->id) % 2)
		{
...
int main(int argc, char **argv) {
	...
	submenu3 = glutCreateMenu(background_menu);
